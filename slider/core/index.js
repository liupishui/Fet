...
$$.printAll=function(obj,stringJiange){
    //target:打印对象的所有属性
...
$$.get=$$.lib.querystring.parse($$.url.query);
...
$$.m=function(){}//每个表将拥有此方法的所有属性
$$.m.prototype.getList=function(num){
    //所有表的增删该查
    var that=this;
    return new Promise(function(resolve,reject){
        $$.connection.query('select * from '+that.table+' order by id desc limit '+num,function(err,data){
                resolve(data);
        });
    });
}
$$.m.prototype.select=function(num){
    //所有表的增删该查
    var that=this;
    return new Promise(function(resolve,reject){
        $$.connection.query('select * from '+that.table+' where id='+num,function(err,data){
                resolve(data);
        });
    });
}
...
//静态目录下文件静态输出
var isStatic=false;
$$.lib.lodash($$.config.staticPath).forEach(function(value,key){
    if($$.url.pathname.indexOf(value)==0){
        isStatic=true;
        var extname=path.extname($$.url.pathname);
        if(extname!=''){
            fs.readFile($$.reqAddress, function(err, data) {
                if(err){
                    res.writeHead(404);
                    res.end();
                }else{
                    res.writeHead(200,{"Content-Type":$$.config.httpHeaders[extname]});
                    res.write(data);
                    res.end();
                }
            });
...
if(!isStatic){
    // ^news/(\d{1,}).html /news/newsview.js?id={r:1}
    ...
    let actionObject=getActionObject();
    if(actionObject){
            //连接数据库
        var mysql=$$.lib.mysql;
        ...
        $$.connection.query('show tables',function(err,data){
            //为每一个表建立对应的对象，并使其拥有增删改查方法
            for(let x in data){
                for(let y in data[x]){
                    $$.tables[data[x][y]]=function(){
                        this.table=data[x][y];
                    }
                    $$.extend($$.tables[data[x][y]],$$.m);
                }
            }
            $$.run=$$.lib.genny.run;
            $$.include('action/index.js');
            $$.run(function *(resume){
                //文件上传及post
                    var form=new $$.lib.formidable.IncomingForm();
                    form.uploadDir = $$.serverPath+'/public/temp/';
                    ...
                    $$.post=result.fields;
                    $$.post.files=result.files;
                    $$.action[actionObject['action']].apply(null,actionObject['arguments']);
            });
        });
    }else{
        res.writeHead(404);
